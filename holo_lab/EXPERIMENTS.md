...
## 後記 (2026-07-07):終端進場穩健化 + 「GUI 為什麼變慢」

### 「帶 GUI 跑比較慢」的對照實驗

手動 `./run_lab.sh`(帶 GUI)跑出 50~90s 的繞路 run,懷疑 run_lab 不如
exp。逐一對照(同一天、同一機器、同一程式碼):

| 模式 | 結果 |
|------|------|
| 整合 GUI (`./run_lab.sh`) | 59.9s / 41m,追蹤比 0.61 |
| 外掛 GUI (`./run_lab.sh gui`) | 90s timeout / 78m |
| **純 headless(對照組)** | **18.0s / 11.9m / eff 1.019 — 完美** |

結論:**WSL2 上 live 模擬掛上 GUI 渲染就會讓 PX4 速度追蹤劣化**(追蹤比
0.61 vs 0.87),路線跟著遊蕩、多繞圈。非演算法問題;exp.sh 與 run_lab.sh
邏輯相同,差別只在 HEADLESS=1。要看畫面就 `./run_lab.sh gui`,但知道它
會比 headless 亂一些。

### 終端進場穩健化:goal_approach_a = 0.5
